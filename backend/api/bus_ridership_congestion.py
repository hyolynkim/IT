# elderly 브랜치에서 만든 "과거 승하차 통계(bus_ridership.csv)" 기반 혼잡도 모듈입니다.
# 같은 폴더의 bus_congestion.py(Normal 브랜치, TAGO 실시간 통계 기반 · 버스 번호
# 검색용)와는 완전히 다른 기능이라 파일명을 분리했습니다. 이 모듈은 노약자/임산부
# 모드의 혼잡도 점수 계산(app.py의 _route_congestion_score)과, 운행하지 않는
